...
def Solve(file):
    ...
    for j in tqdm(range(0, np.size(c.X), 1)):
        ...
        optic_length = 0
        c.x_i[j][0] = c.X[j]
        c.tg[j, 0] = (c.X[j]+15.85E-3)/0.19 #-15.85E-3
        c.dPhi[j] = 0 # 2*np.pi*np.sqrt(0.19*0.19+(c.X[j]-15.85E-3)*(c.X[j])-15.85E-3)/5320*1E10 #-15.85E-3
        c.dPhi_init[j] = c.dPhi[j]
        for i in range(1, c.m + 1):
            tgA[i] = 1 / n_curved(x_temp, i * c.dz + c.dz / 2) * (n_x_curved(x_temp, i * c.dz) * c.dz - n_z_curved(x_temp, i*c.dz) * c.dz * c.tg[j, i])
